...
try {
    ...
    $stmt = $mysqli->prepare("UPDATE user SET name = ?, lastName = ?, mobile = ?, dob = ?, gender = ? WHERE user_id = ?;");
    $stmt->bind_param("sssssi", $name, $lastName, $mobile, $dob, $gender, $user_id);

    $json->emptyField = false;
    // set parameters and execute
    $user_id = $_SESSION['user_id'];
    $name = snatizer($_REQUEST['name'], FILTER_SANITIZE_STRING);
    $lastName = snatizer($_REQUEST['lastName'], FILTER_SANITIZE_STRING);
    $mobile = snatizer($_REQUEST['mobile'], FILTER_SANITIZE_STRING);
    $dob = snatizer($_REQUEST['dob'], FILTER_SANITIZE_STRING);


    $gender =  snatizer($_REQUEST['gender'], FILTER_SANITIZE_STRING);
